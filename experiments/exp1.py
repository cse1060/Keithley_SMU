...
def exp1(inst, arduino, src_voltage, tot_duration, iter_num, readings, emit):
    ...
    delay_time = tot_duration/readings
    for i in range(0, iter_num):
        ...
        for x in range(0, readings):
            ...
            y = inst.query(
                "print(defbuffer1.readings[defbuffer1.endindex])")
            z = inst.query(
                "print(defbuffer1.relativetimestamps[defbuffer1.endindex])")

            data.loc[len(data)] = [i, "ON", x, y, z]

            # inst.write('delay(' + str(delay_time) + ')')
